   ...
   _popupContainer = this.FindByName<ContentView>("PopupContainer");
    ...
    Debug.WriteLine("[HybridPOIPopupOverlay] ?? BackdropOverlay not found in XAML");
       ...
       if (_popupContainer == null)
            Debug.WriteLine("[HybridPOIPopupOverlay] ?? PopupContainer not found in XAML");
     ...
     Debug.WriteLine("[HybridPOIPopupOverlay] ? Initialized as ContentView");
       ...
       {
      Debug.WriteLine($"[HybridPOIPopupOverlay] ? Constructor error: {ex.Message}\n{ex.StackTrace}");
  throw;
   }
        }
   ...
   return;
    ...
    Debug.WriteLine($"[HybridPOIPopupOverlay] ?? ShowPopup START for POI {poi.Id} ({poi.Name}) | Time: {DateTime.Now:HH:mm:ss.fff}");
       ...
       // ? Clear previous popup
                if (_currentPopup != null && _popupContainer != null)
       {
  ...
  // ? Add to visual tree
   ...
   _popupContainer.Content = _currentPopup;
        Debug.WriteLine($"[HybridPOIPopupOverlay] ?? Popup attached to visual tree | Time: {DateTime.Now:HH:mm:ss.fff}");
          ...
          {
    Debug.WriteLine("[HybridPOIPopupOverlay] ? PopupContainer is null - cannot attach popup");
  return;
        }
   ...
   if (_backdropOverlay != null)
          {
                    try
               {
                 await _backdropOverlay.FadeTo(0, 200, Easing.CubicIn);
