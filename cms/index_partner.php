...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM POI" . $whereSql . " ORDER BY CASE IsActive WHEN -1 THEN 1 WHEN 1 THEN 2 WHEN 0 THEN 3 ELSE 4 END ASC, id DESC");
                ...
                <?php else: ?>
                    ...
                    ?>
                        ...
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col group <?php echo ($poi['isActive'] != 1) ? 'opacity-90' : ''; ?>">
                            <div class="h-48 bg-gray-200 relative mb-0">
                                <?php if (!empty($imageUrl)): ?>
                                    <img src="<?php echo htmlspecialchars($imageUrl); ?>" alt="<?php echo htmlspecialchars($poi['name']); ?>" class="w-full h-full object-cover <?php echo ($poi['isActive'] != 1) ? 'opacity-60 mix-blend-overlay' : ''; ?>">
                                <?php else: ?>
