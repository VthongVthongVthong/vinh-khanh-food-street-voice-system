    ...
    catch
{
 ...
 var hasData = hasOfflineData || hasCachedData;
       ...
       if (isOnlineLanguage && !hasData)
          ...
          MainThread.BeginInvokeOnMainThread(() =>
      {
         NarrationPreviewText = $"[?? C?n t?i gói '{GetLanguageName(normalized)}' trong Settings tru?c khi s? d?ng]";
     StatusMessage = $"?? Vui lòng t?i gói '{GetLanguageName(normalized)}' trong Settings d? dùng";
      });
           return;
                }
  ...
  StatusMessage = $"S?n sàng phát... ({normalized.ToUpper()})";
        });
   }
    catch (Exception ex)
   {
...
var normalized = NormalizeLang(language);
 ...
 if (isOnlineLanguage && !hasData)
           {
           StatusMessage = $"? Vui lòng t?i gói '{GetLanguageName(normalized)}' trong Settings tru?c";
           return;
          }

   // ? Can play - data is available (offline or cached)
      _audioManager.AddToQueue(SelectedPOI);
         StatusMessage = "Ðang phát âm thanh...";
  }
        catch (Exception ex)
     {
  ...
  if (string.IsNullOrWhiteSpace(url))
              {
    StatusMessage = "? Không t?o du?c link b?n d?";
    return;
 }

if (!Uri.TryCreate(url, UriKind.Absolute, out var uri))
     {
            StatusMessage = "? Link b?n d? không h?p l?";
      return;
        }
...
var shell = Shell.Current;
        ...
        private void OnAudioStarted(object? sender, POI? poi)
 {
       ...
       {
    IsPlaying = true;
          StatusMessage = poi is null ? "Ðang phát..." : $"Ðang phát: {poi.Name}";
        });
     }
